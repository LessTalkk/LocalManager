...
public class VCardParser_V21 {
    ...
    private static final HashSet<String> sAvailablePropertyNameSetV21 = new HashSet<String>(
            Arrays.asList("BEGIN", "LOGO", "PHOTO", "LABEL", "FN", "TITLE",
                    "SOUND", "VERSION", "TEL", "EMAIL", "TZ", "GEO", "NOTE",
                    "URL", "BDAY", "ROLE", "REV", "UID", "KEY", "MAILER",
                    "NICKNAME"));

    /**
     * Though vCard 2.1 specification does not allow "B" encoding, some data may
     * have it. We allow it for safety...
     */
    private static final HashSet<String> sAvailableEncodingV21 = new HashSet<String>(
            Arrays.asList("7BIT", "8BIT", "QUOTED-PRINTABLE", "BASE64", "B"));
    ...
    /**
     * The encoding type. "Encoding" in vCard is different from "Charset". e.g.
    ...
    // In order to reduce warning message as much as possible, we hold the value
    // which made Logger
    // emit a warning message.
    protected static Set<String> mUnknownTypeMap = new HashSet<String>();
    protected static Set<String> mUnknownValueMap = new HashSet<String>();
    ...
    protected boolean isValidPropertyName(String propertyName) {
        if (!(sAvailablePropertyNameSetV21.contains(propertyName.toUpperCase()) || propertyName
     ...
     * ":" [ws] "VCARD"
     */
    private boolean parseOneVCard(boolean firstReading) throws IOException,
            VCardException {
     ...
     * 
     * @param useCache
     *            When true, line is obtained from mPreviousline. Otherwise,
     *            getLine() is used.
     * @param allowGarbage
    ...
    protected void parseItems() throws IOException, VCardException {
        ...
        try {
            ...
            if (DevEnv.bBackupDebug)
                Log.e(LOG_TAG, "Invalid line which looks like some comment was found. Ignored.", e);
            ended = false;
        } catch (VCardInvalidLineException e) {
            if (DevEnv.bBackupDebug)
                Log.e(LOG_TAG,
                        "Invalid line which looks data invalidate. Ignored.", e);
            ended = false;
        } catch (VCardReachEndException e) {
            if (DevEnv.bBackupDebug)
                Log.e(LOG_TAG, "Reach end of file.", e);
            ended = true;
        } catch (Exception e) {
            if (DevEnv.bBackupDebug)
                Log.e(LOG_TAG, "Invalid line. Ignored.", e);
            ended = false;
        }
        ...
        while (!ended) {
            ...
            try {
                ...
                if (DevEnv.bBackupDebug)
                    Log.e(LOG_TAG,
                            "Invalid line which looks like some comment was found. Ignored.",
                            e);
                ended = false;
            } catch (VCardInvalidLineException e) {
                if (DevEnv.bBackupDebug)
    ...
    protected boolean parseItem() throws IOException, VCardException {
        ...
        if (propertyName.equals("ADR") || propertyName.equals("N")) {
            //            start = System.currentTimeMillis();
    ...
    protected String[] separateLineAndHandleGroup(String line)
            throws VCardException {
    ...
    protected void handleType(final String ptypeval) {
        ...
        if (!(sKnownTypeSet.contains(upperTypeValue) || upperTypeValue.startsWith("X-")) && !mUnknownTypeMap.contains(ptypeval)) {
    ...
    protected String getQuotedPrintable(String firstString) throws IOException,
            VCardException {
    ...
    /**
     * Mainly for "ADR", "ORG", and "N" We do not care the number of strnosemi
     * here.
     * 
     ...
     * semicolon in this string, it must be escaped ; with a "\" character.
     * 
     * We are not sure whether we should add "\" CRLF to each value. For now, we
     * exclude them.
     */
    protected void handleMultiplePropertyValue(String propertyName,
            String propertyValue) throws IOException, VCardException {
        // vCard 2.1 does not allow QUOTED-PRINTABLE here,
        // but some softwares/devices emit such data.
        if (mEncoding.equalsIgnoreCase("QUOTED-PRINTABLE")) {
            propertyValue = getQuotedPrintable(propertyValue);
        }
        if (mEncoding.equalsIgnoreCase("BASE64") || mEncoding.equalsIgnoreCase("B")) {
            propertyValue = getBase64(propertyValue);
        }
        if (mBuilder != null) {
            mBuilder.propertyValues(VCardUtils.constructListFromValue(propertyValue, (getVersion() == VCardConfig.FLAG_V30)));
        }
    }

    /**
     * vCard 2.1 specifies AGENT allows one vcard entry. It is not encoded at
     * all.
     * 
     * item = ... / [groups "."] "AGENT" [params] ":" vcard CRLF vcard = "BEGIN"
     * [ws] ":" [ws] "VCARD" [ws] 1*CRLF items *CRLF "END" [ws] ":" [ws] "VCARD"
     */
    ...
    public boolean parse(Reader is, VCardInterpreter builder) throws IOException, VCardException {
        mReader = new BufferedReader(is, ContactUtil.DEFAULT_BUFFER_SIZE);
        mBuilder = builder;
        if (mBuilder != null) {
            mBuilder.start();
        }
    ...
    //        //        } else {
    //        //            mReader = new BufferedReader(tmpReader, DataEnv.DEFAULT_BUFFER_SIZE);
    //        //        }
    //        mReader = new BufferedReader(tmpReader, DataEnv.DEFAULT_BUFFER_SIZE);
    //        mBuilder = builder;
    //
    ...
    //    public void parse(InputStream is, String charset, VCardInterpreter builder,
    //            boolean canceled) throws IOException, VCardException {
    //        mCanceled = canceled;
    ...
    //            if (DevEnv.bBackupDebug)
    //                Log.d(LOG_TAG, "Total readLine time: " + ((CustomBufferedReader) mReader).getTotalmillisecond() + " ms");
    //        }
    //        if (DevEnv.bBackupDebug) {
    //            Log.d(LOG_TAG, "Time for handling the beggining of the record: " + mTimeReadStartRecord + " ms");
    ...
    //            Log.d(LOG_TAG, "Time for parsing ADR, ORG, and N fields:"
    //                    + mTimeParseAdrOrgN + " ms");
    //            Log.d(LOG_TAG, "Time for parsing property values: "
    //                    + mTimeParsePropertyValues + " ms");
